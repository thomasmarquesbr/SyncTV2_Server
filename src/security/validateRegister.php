<?php
	...
	if (!empty($errors)) {//ERROR Step1
	  // if there are items in our errors array, return those errors
			$data['success'] = false;
			$data['errors'] = $errors;
		  	$data['messageError'] = 'Não foi possível efetuar o cadastro.';
	}elseif (connectDB()){//SUCCESS Step1

		$result = insertUser($_POST['username'], $_POST['email'], $_POST['password']);
		if(strcmp($result, 'success') != 0){//ERROR step2
			...
			$data['success'] = true;
			$data['messageSuccess'] = 'Cadastro efetuado com sucesso!';
		}
